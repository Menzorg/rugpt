...
class RAGService:
    ...
    def _parse_table_rows(
        ...
        Stage 1 — read workbook.
            pylightxl.readxl() returns a Workbook; ws.row() is a method that
            returns rows as lists of raw cell values (strings/ints/floats).
        ...
        *,
        ...
        try:
            ...
            if is_table:
                ...
                logger.info(f"[{fid}] stage={stage}")
                row_embeddings = self._embed_documents(row_texts)
                logger.info(f"[{fid}] embedded {len(row_embeddings)} row vectors")

                stage = "summary_generation"
                logger.info(f"[{fid}] stage={stage}")
                structure_lines = ["Структура таблицы:"]
