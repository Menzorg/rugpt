...
class RAGService:
    ...
    async def ingest(
        ...
        *,
        ...
        content_hash = self._store.hash_bytes(data)
        try:
            existing_doc_id = await self._store.find_doc_id_by_content_hash(content_hash)
        except Exception as exc:
            raise ValueError(f"Duplicate check failed: {exc}") from exc
        if existing_doc_id:
            return {"doc_id": existing_doc_id, "chunks_ingested": 0, "status": "duplicate"}
    ...
    async def find_docs(
        ...
        rows = await self._store.call_search_related_docs(
            org_id=org_id,
            user_id=user_id,
            query=query,
        ...
        return [RelatedDocSearchResult.model_validate(row) for row in rows]
    ...
    async def search_abstract_in_doc(
        ...
        query_embedding = self._embed_query(query)
        rows = await self._store.call_search_abstract_chunks(
            doc_id=doc_id,
            query=query,
            query_embedding=query_embedding,
            top_k=top_k,
        )
        return [ChunkSearchResult.model_validate(row) for row in rows]
    ...
    async def search_concrete_in_doc(
        ...
        query_embedding = self._embed_query(query)
        rows = await self._store.call_search_concrete_chunks(
            doc_id=doc_id,
            query=query,
            query_embedding=query_embedding,
            top_k=top_k,
