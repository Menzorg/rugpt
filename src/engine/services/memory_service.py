...
_SUMMARY_SYSTEM_PROMPT = (
    ...
    "Составь сжатое, информативное резюме на русском языке: "
    "о чём говорили, какие решения были приняты, какие задачи упомянуты. "
...
class MemoryService:
    ...
    async def generate_summary(
        ...
        if previous_summary:
            system_text += (
                f"\n\nУчитывая предыдущую суммаризацию диалога, которая охватывает "
                f"историю дальше, чем {n} данных сообщений:\n\n{previous_summary}"
            )
