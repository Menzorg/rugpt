...
class AgentExecutor:
    ...
    async def execute(
        ...
        system_prompt += (
            "\n\n##ВАЖНЫЕ ОГРАНИЧЕНИЯ\n"
            "Запрещены любые фразы: \"ожидайте отчёта\", \"Я продолжаю поиск\", \"сейчас обработаю остальные N документов\", \"позже сообщу результаты\".\n"
            "Запрещено писать \"я приступаю\", \"начинаю поиск\", \"приступаю к анализу\".\n"
            "Если лимит вызовов инструментов исчерпан — сообщи что удалось найти, и что для более полного ответа нужен новый запрос."
            "Написав текст без вызова инструмента, ты завершаешь задачу — пользователь не получит ничего после этого сообщения."
        )
