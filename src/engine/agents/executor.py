...
class AgentExecutor:
    ...
    async def execute(
        ...
        system_prompt += (
            ...
            f"- в чате сообщения разных ассистентов маркируются по системному имени отправителя. Твоё имя: {role.code}. В ответах пользователя запрещается напрямую давать ответы в тегах name, content."
        )
