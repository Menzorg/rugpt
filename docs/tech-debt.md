...
| # | Проблема | Приоритет | Описание |
|---|----------|-----------|----------|
| ~~9a~~ | ✅ ~~File upload без signature~~ — **CLOSED (2026-05)** | — | `POST /files/upload` подписывается end-to-end: фронт — `apiClient.signedUpload` с `file_sha256` + `route_id`; NestJS — `signedRelay.forwardMultipart`; Engine — `WebSignatureMiddleware` пересчитывает `file_sha256` из принятых байт и верифицирует подпись. Украденного JWT без устройства недостаточно. |
| ~~9b~~ | ✅ ~~File download без signature~~ — **CLOSED (2026-06-03)** | — | Бэк: `SignedRelayService.forwardBinary` + `RuGPTEngineAdapter.relaySignedBinary` подписывают и проксируют GET в Engine как стандартный signed envelope (`signature/nonce/sig_timestamp/user_id/route_id` в query). Фронт: `apiClient.signedGetBlob` + новый `httpClient.getBlob` (`useFiles`, `useAuthenticatedFile`, `MessageBubble` переключены). Удалён мёртвый `FileService` целиком + action `'download_file'` в адаптере. Доказано curl-тестом: unsigned→401, signed→200 байт-в-байт, tampered signature→Engine 401 «Invalid device signature». |
| 37 | Adapter оборачивает Engine HTTP-4xx в 503 | Средний | `RuGPTEngineAdapter.handleError` (`packages/backend/src/engine/adapters/rugpt.adapter.ts:52+`) возвращает `[false, message]` для **любой** не-2xx Engine-ответа (4xx и 5xx одинаково), не отличая «transport-failure» от «Engine ответил с error-кодом». Следствие: `SignedRelayService.forward()`/`forwardMultipart()`/`forwardBinary()` бросают `ServiceUnavailableException` (503) даже когда Engine осознанно вернул 401 «Invalid signature», 403 «Access denied», 404 и т.д. Клиент видит «сервис недоступен» вместо реальной причины. Безопасность не страдает (запрос отвергается), UX страдает; фронт не может различить «попробуй ещё раз» от «у тебя нет прав». **Фикс:** в `handleError` различать `error.response?.status` (4xx → success=true с {status, body}) vs network-error (ECONNREFUSED/ETIMEDOUT → success=false). `forwardX()` тогда мапит engine-status в соответствующий NestJS-exception. Найдено при closure tech-debt 9b (см. `tasks/files_remove_skip_signature.md` Task 6). |
...
| 36 | Внутренний nginx `10.0.0.2` `listen 80;` — открыт всему LAN, не только VPN | Средний | На rugpt-container nginx слушает все интерфейсы (включая LAN-адрес `.81:80`), хотя единственный легитимный клиент — Prod-VPS через WireGuard. Хост из LAN `192.168.1.0/24` (Zver, postgres-vm, gitlab-runner, …) теоретически дотягивается напрямую и шлёт запросы в обход VPN. Сейчас прикрыто только Omada-ACL на уровне VPN-пиров, на LAN-стороне ACL нет. В Plan A контекст менее острый (engine сам делает crypto-verify через `WebSignatureMiddleware`, доверительного identity-заголовка для подмены нет), но replay/DoS через LAN всё ещё возможен. **Решение (solve-by-construction):** `listen 10.0.0.2:80;` вместо `listen 80;` — единственный путь к nginx через WireGuard от Prod-VPS, LAN-поверхность закрыта на сетевом уровне. Перед применением — согласовать с лидом, не сломает ли других легитимных потребителей (CI-деплой, LAN-healthcheck'и). Источник: ревью таска `user_id_from_signature` (C4, done upstream 2026-06-01), оригинальный «вариант B». |
...
| ~~31~~ | ✅ ~~`KAFKA_ENABLED=false` sync fallback в AIService~~ — **CLOSED (2026-06)** | — | Закрыт задачей `tasks/kafka_mandatory_remove_sync_fallback.md`. Удалён флаг `Config.KAFKA_ENABLED` и все sync-ветки (`try_auto_respond`, `process_ai_mentions`, `_is_async_mode`); агентные вызовы идут единственным путём через `agent.requests`. На старте — fail-fast: `KafkaProducerService.start()` бросает исключение → Engine не поднимается, если брокер недоступен. SLA-стэмп support-тикетов (жил только в sync-ветке → в проде не работал) перенесён в `AgentRequestHandler` (`record_support_first_response`). Тесты переведены на мок-продюсер. |
| ~~32~~ | ✅ ~~Identity на ~114 endpoint'ах берётся из JWT, а не из signed payload~~ — **CLOSED (2026-05, Plan A)** | — | Закрыт fast-fix'ом из исходного описания: `get_current_user` (`routes/auth.py:93+`) теперь требует, чтобы `request.state.zt_user_id` (выставленный `WebSignatureMiddleware` после крипто-верификации подписи) совпадал с `JWT.user_id`, иначе 401 «Identity mismatch»; нет zt_user_id (запрос без подписи) → 401 «Unsigned request». Действует для всех `/api/v1/web/*`-роутов разом, не по-роутово, без переписывания endpoint'ов. «Угон JWT без устройства» больше не работает на /web-фасаде. Полная миграция на `user_id: UUID` query-параметр + storage lookup — больше не обязательна, отдельная задача только если потребуется убрать JWT-фасад целиком. |
